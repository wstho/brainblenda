...
def random_RGBA(low = 0, high = 100, alpha = 1):
...
def flip_mesh_across_midline(obj, midline=5691.66):
    ...
    obj.scale.z *= -1
    if obj.parent:
        obj.parent.location = [obj.parent.location[0], obj.parent.location[1], 2*midline - obj.parent.location[2]]
...
def build_from_swc(filepath, name = None,
                  lx=0, ly=0, lz=0,
                  coord_space = None, flip = False, midline = 5691.66,
                  rotate = False, rotating = False, draw_axon = False, 
                  fill_process_tips = False, merge_threshold = 0.001, 
                  scale_f = 1, colour = None, alpha = 1):
    ...
    for key, value in neuron.items():
        if (value[0] == 1):                                 #soma
            somaRadie = somaR
            bpy.ops.mesh.primitive_uv_sphere_add(location=(0 / scale_f, 0 / scale_f, 0 / scale_f), radius=somaRadie / scale_f, segments=64, ring_count=64)
        ...
        if (value[-1] != last):
            if fill_process_tips:
                if (last != -10): 
                    bpy.ops.mesh.primitive_uv_sphere_add(radius=p.radius , location= p.co , scale=(1, 1, 1), segments=32, ring_count=16)
    ...
    bpy.ops.mesh.remove_doubles(threshold=merge_threshold, use_unselected=False, use_sharp_edge_from_normals=False)
    bpy.ops.object.mode_set(mode = 'OBJECT')
...
def import_allen_mesh(filepath, forward_axis ='Y', up_axis = 'Z', colour = None, alpha = 1):
...
def mesh_from_coordinates(coordinates, name = None, reference_object = None, scale_f = 1):
    ...
    if not reference_object: 
        ...
        bpy.ops.mesh.primitive_uv_sphere_add(location=[0,0,0], radius=10, segments=16, ring_count=16)
...
def draw_somas(soma_coordinates, name = None, colour = None, radius = 7, res = 1, alpha = 1, scale_f = 1e6):
    ...
    pbsdf_node.inputs['Base Color'].default_value = colour
    pbsdf_node.inputs['Alpha'].default_value = alpha
    bpy.ops.mesh.primitive_uv_sphere_add(location=[0,0,0], radius=radius, segments=int(8*res), ring_count=int(8*res))
    reference_sphere = bpy.context.selected_objects[0]
    reference_sphere.data.materials.append(material)
    
    mesh_object = mesh_from_coordinates(soma_coordinates, name = name, reference_object = reference_sphere, scale_f = scale_f)
...
def draw_neuron_from_snudda(neurons, neuron_id, snudda_data, name = None, colour = None, scale_f = 1e6):
    ...
    build_from_swc(snudda_parse_path(neurons[neuron_id]['morphology'], snudda_data), name = name, lx = position[0], ly = position[1], lz = position[2], colour = colour) 
...
def draw_postsynaptic(network_path, post_id, snudda_data, draw_presynaptic_partners = True, draw_synapses = True, scale_f = 1e6, post_colour = (239/255, 42/255, 126/255, 1.0), pre_colour = None, synapse_colour = None, synapse_radius = 5, match_synapses = False):
    ...
    draw_neuron_from_snudda(neurons, post_id, snudda_data, name = 'postsynaptic' + str(post_id), colour = post_colour)
    
    if not match_synapses:

        if draw_presynaptic_partners:
            for pre_id in pre_ids:
                draw_neuron_from_snudda(neurons, pre_id, snudda_data, name = 'presynaptic'+ str(pre_id), colour = pre_colour)
        ...
        if draw_synapses:
            ...
            bpy.ops.mesh.primitive_uv_sphere_add(location=[0,0,0], radius=synapse_radius, segments=16, ring_count=16)
            reference_sphere = bpy.context.selected_objects[0]
            reference_sphere.data.materials.append(material)

            mesh_from_coordinates(pre_synapse_coords, name = 'synapses', reference_object = reference_sphere, scale_f = scale_f)
                            
    else:
        
        for pre_id in pre_ids:
            
            matched_synapses = ((synapses[:,0] == pre_id) & (synapses[:,1] == post_id)).astype(bool)

            colour = random_RGBA(low = 0, high = 255)
            draw_neuron_from_snudda(neurons, pre_id, snudda_data, name = 'presynaptic'+ str(pre_id), colour = colour)
            ...
            mesh_from_coordinates(synapse_coords[matched_synapses], name = 'synapses' + str(pre_id), reference_object = reference_sphere, scale_f = scale_f)
            
    return pre_ids, pre_synapse_coords


def draw_presynaptic(network_path, pre_id, snudda_data, draw_postsynaptic_partners = True, draw_synapses = True, scale_f = 1e6, pre_colour = (239/255, 42/255, 126/255, 1.0), post_colour = None, synapse_colour = None, synapse_radius = 5):
    ...
    if draw_postsynaptic_partners:
        for post_id in post_ids:
            draw_neuron_from_snudda(neurons, post_id, snudda_data, name = 'postynaptic'+ str(post_id), colour = post_colour)
...
def add_virtual_input_locations(coords, neuron, location = [0,0,0], rotation = [0,0,0], name = None, scale_f= 1, soma_radius =10, colour = None, radius = 4,  res = 1, jitter_amount = None, timing = None): 
    ...
    if timing:
        for c in coords: 
            ...
            sphere.keyframe_insert(data_path="location", frame=1, options={'INSERTKEY_NEEDED'})
            sphere.location = rotated_coord + mathutils.Vector(location)
            sphere.keyframe_insert(data_path="location", frame=random.randint(timing - 5, timing + 5), options={'INSERTKEY_NEEDED'})
...
if __name__ == '__main__':
    print('Do not run this file directly. Call functions from python within blender please!')
    sys.exit(-1)